...
```{r}
plot_transform(
  m = matrix(c(1, 0, 0), nrow = 2),
  matrix(c(1, -1, 1, 1), nrow = 2)
)
```

## 5. 
$cos(x)=.8sin(x)=.6$, so take the inverse to get in radians: `r asin(.6)/(2*pi)`

## 6,7.

If the line is $\m{2\\1\\2}$ and the line $\m{1\\1\\1}$, then the orthogonal projection is $\frac{5}{9} \m{2\\1\\2}$. The reflection over this line is the vector minus twice its component orthogonal to the line, which is the vector minus its _parallel_ component. This rearranges to twice $x^{\perp}- x$.
\[refl=\m{2\\1\\2} - 2(\m{2\\1\\2}-\frac{5}{9} \m{2\\1\\2})\]
\[refl= 2(-\frac{5}{9} \m{2\\1\\2}))- \m{2\\1\\1}\]


8. \[\m{0 &-1\\-1 & 0}\] is a dual shear.

9. \[\m{ 1&0\\1&1 }\] is a vertical shear with no scaling.

10. By the formulas, the matrix of the projection onto $\m{4\\3}$ is 
\[\frac{1}{25}\m{16&12\\
16&9}\]
...
The reflection is $2P-I$. The unit vector on this line is $\m{4/5\\3/5}$, so we compute the projection onto that vector:
...
\[\m{(\cos{\theta}-sin{\theta})(u_1^2 +u_1u_2)&
(\cos{\theta}-sin{\theta})(u_2^2 +u_1u_2)\\
(\sin{\theta}+cos{\theta})(u_1^2 +u_1u_2)
(sin{\theta}+\cos(\theta)(u_2^2u_1+u_1u_2)}\]

A vector for which $Av=v$ lies on the line of reflection and is a multiple f$\m{b-1//b}$
...
by the unit-length constraint, an identity, and also:

\[\begin{aligned}
&u_1^3u_2+u_1u_2^3\\
&u_1u_2(u_1^2+u_2^2)\\
&u_1u_2u_1u_2\end{aligned}\]
...
Should two matrices have a defined product, row $i$ of AB is the dot product of $A_i$ with each $B_j$.
...
## 59. 
How to "invert" a linearly dependent matrix...you can't, because if no inverse exists $A^{-1}A=I$ has no solution.
...
\[x_1+2x_2=0\]
\[x_1+2x_2=1\]
Because a dimension is collapsed $(ad=bc)$, you can't distinguish between diagonals and off-diagonals to get the identity.
...
If $A$ is positive and $B$ is not, their product in either direction must be positive.
...
noninvertible because undefined for $\infty,0$.
...
\[\begin{aligned}&(A^2)^{-1}=(A^{-1})^2\\
&(AA)^{-1} = 5A^{-1}A^{-1}\\
&A^{-1}A^{-1}=A^{-1}A^{-1}\end{aligned}\]

# 72.

\[\begin{aligned}&ABA^{-1}=B\\
&BA^{-1}\neq{A^{-1}B}
\end{aligned}\]
...
\[\begin{aligned}&sB_{j1}+sB{j2}+\dots4B_{jn}\\
...
\[\begin{aligned}&A_{kk}=A_1 A_2+\dots+A_n A_n\\
&=\sum_{i=1}^{n}A_i^2\\
&=r^2
\end{aligned}\]
...
\[\begin{aligned}&(I_n-A)(I_n+A+A^2+\dots+A^m)\\
