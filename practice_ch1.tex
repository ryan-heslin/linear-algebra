...
configurations for a \(3\times 2\) RREF: none, \((1, 1)\) and
\((2, 2)\), \((1, 1)\) and \((2, 3\)) \# System of a Line
...
\KeywordTok{solve}\NormalTok{(}
  \DataTypeTok{a =} \KeywordTok{matrix}\NormalTok{(}\KeywordTok{c}\NormalTok{(}
    \DecValTok{1}\NormalTok{, }\FloatTok{-.2}\NormalTok{, }\FloatTok{-.3}\NormalTok{,}
    \FloatTok{-.1}\NormalTok{, }\DecValTok{1}\NormalTok{, }\FloatTok{-.4}\NormalTok{,}
    \FloatTok{-.2}\NormalTok{, }\FloatTok{-.5}\NormalTok{, }\DecValTok{1}
\NormalTok{  ), }\DataTypeTok{byrow =} \OtherTok{TRUE}\NormalTok{, }\DataTypeTok{nrow =} \DecValTok{3}\NormalTok{),}
...
\begin{Highlighting}[]
\KeywordTok{solve}\NormalTok{(}
  \DataTypeTok{a =} \KeywordTok{square}\NormalTok{(}\DecValTok{1}\NormalTok{, }\DecValTok{1}\NormalTok{, }\DecValTok{1}\NormalTok{, }\DecValTok{1}\NormalTok{, }\DecValTok{3}\NormalTok{, }\DecValTok{9}\NormalTok{, }\DecValTok{1}\NormalTok{, }\DecValTok{1}\NormalTok{, }\DecValTok{2}\NormalTok{),}
...
\section{1.2}\label{section-7}}

\hypertarget{section-8}{%
\subsection{19.}\label{section-8}}
...
\subsection{32.}\label{section-10}}
...
\includegraphics{practice_ch1_files/figure-latex/unnamed-chunk-8-1.pdf}
...
\subsection{47.}\label{section-13}}

Infinite: \(k=2\) Inconsistent: \(k= 1\) Otherwise: unique

\hypertarget{section-14}{%
\subsection{48.}\label{section-14}}
...
\subsection{66.}\label{section-15}}
...
\subsection{80.}\label{section-19}}
...
\NormalTok{A <-}\StringTok{ }\NormalTok{matador}\OperatorTok{::}\KeywordTok{square}\NormalTok{(}
  ...
  \DecValTok{-2}\NormalTok{, }\DecValTok{5}\NormalTok{, }\DecValTok{-3}
\NormalTok{)}
...
\section{1.3}\label{section-20}}

\hypertarget{section-21}{%
\subsection{29-32.}\label{section-21}}

Let \(x=\begin{bmatrix}5\\3\\-9\end{bmatrix}\) and
\(b=\begin{bmatrix}2\\0\\1\end{bmatrix}\). To find \(A\) from these
vectors, we just solve the system of equations \[ \begin{aligned}
&5a_1+3a_2-9a_3=b_n\\
&a_1=\frac{b_2-3a_2-9a_3}{5}
\end{aligned}\] substituting each element of \(A\) for the corresponding
row of \(A\). The rank 1 \(A\):
...
\subsection{49.}\label{section-23}}
...
\(3 \times 4\) rank 3: augment rank 3, since full row rank guarantees a
solution, so \(AX=b\) for all \(b\) Infinitely many solutions. \#\#
Costs and Units
...
is: \[\begin{aligned}
& x+y+z=32\\
& x+5y+10z=100 \end{aligned}\] Row reduce to get \[ \begin{aligned}
& x = 5/4z+15\\
& y=17-9/4z
\end{aligned}\]
...
\item
  ...
  expressed \(r_i=c_1r_j+c_2r_k\) Let \(b_i\) be a linear combination of
  \(b\) with different scalars such that
...
\subsection{46.}\label{section-24}}
...
\subsection{67.}\label{section-32}}

If we add the constraint \(a +b=\), we have rotations.

\hypertarget{section-33}{%
\subsection{68.}\label{section-33}}
...
\item
  True; the product has dimension of the LHS.
\item
...
\StringTok{  }\KeywordTok{suppressWarnings}\NormalTok{(purrr}\OperatorTok{::}\KeywordTok{map}\NormalTok{(}\OperatorTok{~}\StringTok{ }\KeywordTok{try}\NormalTok{(}\KeywordTok{solve}\NormalTok{(.x)))) }\OperatorTok{%>%}
\StringTok{  }\NormalTok{purrr}\OperatorTok{::}\KeywordTok{map_lgl}\NormalTok{(is.matrix) }\OperatorTok{%>%}
